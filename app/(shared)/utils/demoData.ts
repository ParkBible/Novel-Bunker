...
export async function initializeDemoData() {
    // 작품이 하나라도 있으면 초기화 생략
    const existingProjects = await projectOps.getAll();
    if (existingProjects.length > 0) {
        return;
    }

    // 데모 작품 생성 (기본 챕터 1개가 함께 만들어짐 → 삭제 후 데모 챕터로 대체)
    const { projectId, firstChapterId } =
        await projectOps.create("글을 쓰는 사람들");
    await chapterOps.delete(firstChapterId);

    await projectOps.update(projectId, {
        synopsis: `《글을 쓰는 사람들》
...
믿음과 좌절, 열정과 현실 사이에서 균형을 찾아가는 젊은 작가의 여정을 그리며, 창작의 고통과 기쁨, 그리고 사람들과의 연결을 통해 진정한 작가로 성장해가는 과정을 담았다.`,
    ...
    await relationshipOps.create(projectId, seoyunId, minhoId, "원고 발견");
    await relationshipOps.create(projectId, seoyunId, jiwooId, "멘토-멘티");

    // Create chapters
    const chapter1 = await chapterOps.create(projectId, "프롤로그");
    const chapter2 = await chapterOps.create(projectId, "첫 번째 만남");

    // Create scenes for chapter 1
    await sceneOps.create(chapter1, projectId, "카페의 아침", 0);
    await sceneOps.update(
        await sceneOps.create(chapter1, projectId, "우연한 발견", 1),
        {
...
하지만 이번엔 다를 것 같았다. 손끝에서 느껴지는 이 떨림, 이야기가 살아 숨 쉬는 이 감각. 마침내 제대로 된 이야기를 쓰고 있다는 확신이 들었다.`,
    ...
    await sceneOps.create(chapter2, projectId, "출판사에서", 0);
    await sceneOps.update(
        await sceneOps.create(chapter2, projectId, "편집자의 제안", 1),
        {
...
그의 목소리에는 흥분이 섞여 있었다. 10년간 편집자로 일하며 수천 편의 원고를 봤지만, 이런 느낌은 처음이었다.`,
    ...
    await sceneOps.update(
        await sceneOps.create(chapter2, projectId, "멘토의 조언", 2),
        {
