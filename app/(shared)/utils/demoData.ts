...
export async function initializeDemoData() {
    ...
    await sceneOps.update(await sceneOps.create(chapter1, "우연한 발견", 1), {
...
하지만 이번엔 다를 것 같았다. 손끝에서 느껴지는 이 떨림, 이야기가 살아 숨 쉬는 이 감각. 마침내 제대로 된 이야기를 쓰고 있다는 확신이 들었다.`,
    ...
    await sceneOps.update(await sceneOps.create(chapter2, "편집자의 제안", 1), {
...
그의 목소리에는 흥분이 섞여 있었다. 10년간 편집자로 일하며 수천 편의 원고를 봤지만, 이런 느낌은 처음이었다.`,
    ...
    await sceneOps.update(await sceneOps.create(chapter2, "멘토의 조언", 2), {
